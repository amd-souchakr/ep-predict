...
def decode_dispatch_inputs(
    ...
    *,
    ...
    histogram = (
        _field(routing_data, "expt_hist", "hist")
        .detach()
        .to("cpu", torch.int64)
        .reshape(-1)
    )
    ...
    gather_src = (
        _field(gather_idx, "src_indx", "src_idx")
        .detach()
        .to("cpu", torch.int64)
        .reshape(-1)
    )
    gather_dst = (
        _field(gather_idx, "dst_indx", "dst_idx")
        .detach()
        .to("cpu", torch.int64)
        .reshape(-1)
    )
    ...
    if histogram.sum().item() != expected_slots:
        raise ValueError(
            f"dispatch histogram covers {histogram.sum().item()} slots, expected {expected_slots}"
        )
...
def canonicalize_pairs(
    ...
    return torch.gather(expert_ids.to(torch.int64), -1, order), torch.gather(
        weights, -1, order
    )
...
def compare_routes(
    ...
    *,
    ...
    expected_ids, expected_weights = canonicalize_pairs(
        expected_ids.detach().cpu(), expected_weights.detach().cpu()
    )
    consumed_ids, consumed_weights = canonicalize_pairs(
        consumed_ids.detach().cpu(), consumed_weights.detach().cpu()
    )
    if (
        expected_ids.shape != consumed_ids.shape
        or expected_weights.shape != consumed_weights.shape
    ):
        return {
            "id_mismatches": max(expected_ids.numel(), consumed_ids.numel()),
            "weight_mismatches": max(
                expected_weights.numel(), consumed_weights.numel()
            ),
    ...
    return {
        ...
        "weight_mismatches": int(weight_error.gt(weight_atol).sum().item()),
        "max_abs_weight_error": float(weight_error.max().item())
        if weight_error.numel()
        else 0.0,
    }


def expected_from_logits(
